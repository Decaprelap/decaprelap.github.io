...
var SoundCloudAudioSource = function(player) {
      ...
      var sampleAudioStream = function() {
          ...
          var totalLow = 0;
          for (var i = 0; i < 39; i++) { // get the volume from the first 80 bins, else it gets too loud with treble
              totalLow += self.streamData[i];
          }
          self.volumeLow = totalLow;
          
          var totalHi = 0;
          for (var i = 39; i < 80; i++) { // get the volume from the first 80 bins, else it gets too loud with treble
  ...
  var Visualizer = function() {
      var audioSource;
      var mental = {now:0, prev:0};
      var draw = function() {
          // Beat
          mental.now = (Math.min(Math.max((Math.tan(audioSource.volumeHi/6500) * 0.5), -20), 2) * -1);
          $('.scope ul li').each(function(count){
            $(this).css('height', audioSource.streamData[count]);
          });
          if ((mental.now - mental.prev) > 0.005) {
            $('.fullscreen').css('background', '#111');
  ...
  var SoundcloudLoader = function(player,uiUpdater) {
      var self = this;
      var client_id = "7ff7507da281c083017c7ffb499b1955"; // to get an ID go to https://developers.soundcloud.com/
      this.sound = {};
      this.streamUrl = "";
      this.errorMessage = "";
      this.player = player;
